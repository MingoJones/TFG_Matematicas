...
El siguiente resultado sobre integración Riemann lo hemos utilizado en el Ejemplo \ref{E2.3}, que es a su vez un ejercicio en \cite[Exer 1.14]{kuipers-1974}. De nuevo, incluimos su prueba por completitud.
...
\begin{proof}
    ...
    Fijado $\varepsilon > 0$, definimos funciones lineales a trozos $L$ y $U$ en $[a,1)$ de la siguiente forma. 
    
    Para cada intervalo $[a_{i-1}, a_i)$,  
    \[
    ...
    \]
    Ahora, observamos que $a_{i-1} \geq a$, 
    \[
    ...
    cada $i=1,\ldots,k$. Además, como $\max_{1 \leq i \leq k} (a_i - a_{i-1}) < \delta'$
    \[
        \sum_{i=1}^k \frac{M_i-m_i}{2a}(a_i-a_{i-1})^2 <
        \sum_{i=1}^k \delta' \frac{B}{a}(a_i-a_{i-1})= 
        \delta' \frac{B}{a} (1-a) =
        \delta' K,
    \]
    siendo $K = \frac{B}{a} (1-a)$ una constante que no depende de la partición. 
    
    Por lo tanto,
    \[
        \int_a^1 U(x) \, dx - \int_a^1 L(x) \, dx < 
        \int_a^1 u(x)\,dx - \int_a^1 l(x)\,dx + \delta' K.
    \]

    Sea ahora $\delta > 0$ tal que $\delta < \delta'$ y que cumpla $\delta K < \varepsilon$. 
    Si tomamos una partición $P$ con $\max_{1 \leq i \leq k} (a_i - a_{i-1}) < \delta$, tendríamos que
    \[
        \int_a^1 u(x) \, dx - \int_a^1 l(x) \, dx < \varepsilon
        \quad \text{y} \quad 
        \delta K < \varepsilon.
    \]
    La diferencia de las integrales de $L$ y $U$ en $[a,1)$ se puede acotar por
    \[
        \int_{a}^1 U(x) \, dx - \int_{a}^1 L(x) \, dx < 
        2\varepsilon.
    \]
