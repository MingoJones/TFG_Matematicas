...
\begin{proof}
    ...
    Para cada $h=1,2,\dots,$ definimos la función \bl{
    \[
   ...
   % \]
    ...
    Comprobemos que $f_h^{(k-1)}(x)$ es monótona para $x \geq x_0$. Para esto, 
    %basta con mostrar que su derivada es de signo constante. 
    \bl{usando la 
    expresi\'on integral anterior tenemos
    \[
        f_h^{(k-1)}(x) = 
        %g^{(k)}(x+h) - g^{(k)}(x)
        \int_0^1g^{(k)}(x+sh)\,h\,ds,
    \] y}
    como $g^{(k)}(x)$ es monótona para $x \geq x_0$, entonces \bl{tambi\'en lo es $f_h^{(k-1)}(x)$}. En~consecuencia, $f_h^{(k-1)}(x)$ es monótona para $x \geq x_0$.
