...
\begin{resolve}
    ...
    Para comprobar la monotonía de $g^{(k)}$, calculamos $g^{(k+1)}$:
    \[
        g^{(k+1)}(x) = a \,\frac{\log^{\tau-k-1}(x+1)}{(x+1)}\left(Q(\log(x+1))\right),
    \]
    donde $Q$ es un polinomio de grado $k+1$ cuyo coeficiente principal es $\tau k!$.

    Dado que $\frac{\log^{\tau-k-1}(x+1)}{(x+1)} > 0$ para cada $x \geq 1$ y que
    \[
        \lim_{x \to \infty} Q(x) = -\infty,
    \]
    debido a que el coeficiente del término de mayor grado es $\tau k!< 0$,
