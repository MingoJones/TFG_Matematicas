...
\begin{proof}
    Sea $u_n \in \mathbb{C}$ con $n = 1, \dots, N$. Definimos $u_n = 0$ para $n > N$ y para $n \leq 0$. 
    Entonces,
    \[
    ...
    Ahora elevamos al cuadrado ambos lados y aplicamos la desigualdad de Cauchy-Schwarz,
    \[
    ...
    la segunda es la parte inferior $k < s$ y la tercera es la parte superior $k > s$. Por lo tanto,
    \[
        \sum_{k=1}^{H} \sum_{s=1}^{H} u_{p-k} \overline{u_{p-s}} =
        \sum_{k=1}^{H} |u_{p-k}|^2 \, +
    ...
    como $p=2,\dots,N+H$ y $s=2,\dots,H$ entonces $n=0,\dots,N$. Además, como la suma es finita
    podemos cambiar el orden de los sumatorios y se obtiene,
    \[
        \sum_{p=2}^{N+H} \sum_{1 \leq k < s \leq H} u_{p-k} \overline{u_{p-s}} =
        \sum_{1 \leq k < s \leq H} \sum_{n=0}^{N} u_{n+s-k} \overline{u_n}
    \]
    Ahora, sea $k$ fijo, hacemos el cambio de variable $h = s-k$. Como $k < s$ entonces $s = k+1, \dots, H$ y 
    $h = 1, \dots, H-k$. Por lo tanto,
    Obtenemos,
    \[
        \sum_{k=1}^{H-1} \sum_{h=1}^{H-k} \sum_{n=0}^{N} u_{n+h} \overline{u_n}
    \]
    Sabemos que $1 \leq h \leq H-k$ entonces $2 \leq h+k \leq H$ y $1 \leq k \leq H-h$,
    \begin{align*}
        \sum_{k=1}^{H-1} \sum_{h=1}^{H-k} \sum_{n=0}^{N} u_{n+h} \overline{u_n}  
        &=\sum_{k=1}^{H-1} \sum_{h=1}^{H-1} \mathbf{1}_{[2,H]}(h+k) \left(\sum_{n=0}^{N} u_{n+h} \overline{u_n} \right) \\
        &=\sum_{h=1}^{H-1} \sum_{k=1}^{H-1} \mathbf{1}_{[2,H]}(h+k) \left(\sum_{n=0}^{N} u_{n+h} \overline{u_n} \right) \\
        &=\sum_{h=1}^{H-1} \sum_{k=1}^{H-h} \left(\sum_{n=0}^{N} u_{n+h} \overline{u_n} \right) \\
        &=\sum_{h=1}^{H-1} (H-h) \left(\sum_{n=0}^{N} u_{n+h} \overline{u_n} \right)
     \end{align*}
    ...
    Tomando las acotaciones $N+H-1 \leq 2N$, $H-h \leq H$, $\operatorname{Re}(z) \leq |z|$ 
    para todo $z \in \mathbb{C}$. Además, teniendo en cuenta que $u_n = 0$ para $n > N$ y para $n \leq 0$, se obtiene,
    \[
        H^2 \left| \sum_{n=1}^{N} u_n \right|^2 \leq
        4N H \sum_{h=0}^{H-1} \left| \sum_{n=1}^{N-h} u_{n+h} \overline{u_n} \right|
    \]

    Finalmente, dividiendo ambos lados por $H^2$ y aplicar $u_n = e^{2\pi i f(n)}$se obtiene la cota deseada,
    \[
        \left| \sum_{n=1}^N e^{2\pi i f(n)} \right|^2 \leq
        4 \, \frac{N}{H} \sum_{h=0}^{H-1} \left| \sum_{n=1}^{N-h} e^{2\pi i (f(n+h) - f(n))} \right|
    \]
