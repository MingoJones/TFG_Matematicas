...
\begin{proof}
    ...
    \]
    Sea $k$ fijo, consideramos el cambio de variable $p = n+k$. Luego, $n = p-k$ y $p=1+k,\dots,N+k$
    \[
    ...
    Fijado $p$, desarrollamos el sumatorio de dentro del término de la derecha:
    \[
    ...
    \]
    Este doble sumatorio se descompone en tres partes: la parte diagonal, correspondiente a $k=s$;
    la parte inferior, correspondiente a $k < s$; y la parte superior $k > s$,
    \[
        \sum_{k=1}^{H} \sum_{s=1}^{H} u_{p-k} \overline{u_{p-s}} =
        \sum_{k=1}^{H} |u_{p-k}|^2 \, +
    ...
    Estudiamos el sumatorio del segundo término. Hacemos el cambio de variable $n = p-s$,
    ...
    \begin{align*}
     ...
     La cota anterior se simplifica como
     \begin{align*}
        H^2 \left| \sum_{n=1}^{N} u_n \right|^2 
    ...
    Considerando las siguientes acotaciones: $N+H-1 \leq 2N$, $H-h \leq H$ y 
