...
\begin{resolve}
    ...
    no puede converger a $0$, ya que oscila sobre una circunferencia de radio positivo.

    El término central se puede acotar por
    \[
        \left|\frac{1}{2} \left(e^{2\pi i a \log N} + 1\right)\right| \leq 1,
    \]
    luego, tras dividir por $N$, támbien tiende a $0$.
    ...
    En consecuencia,
    \[
        \lim_{N \to \infty} \frac{1}{N} \int_1^N \left(\partfrac{x} - \frac{1}{2}\right) \dfrac{2\pi i a}{x} \, e^{2\pi i a \log x} \, dx = 0.
    \]

    Como la primera integral no converge a $0$, no se verifica el criterio de Weyl para $k=1$.
